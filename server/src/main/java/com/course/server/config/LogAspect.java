...
public class LogAspect {
    ...
    public void doBefore(JoinPoint joinPoint) throws Throwable {
        // 日志编号（日志跟踪编号，需要在logback.xml中配置Pattern）
        // <!-- 带8位UUID的日志 -->
        // <Pattern>%d{mm:ss.SSS} %highlight(%-5level) %blue(%-30logger{30}:%-4line) %green(%-8X{UUID}) %msg%n</Pattern>
        ...
        // SerializerFeature.WriteMapNullValue 这个会打印为null的属性
        LOG.info("请求参数: {}", JSONObject.toJSONString(arguments, excludefilter, SerializerFeature.WriteMapNullValue));
    }
    ...
    public Object doAround(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        // LOG.info("请求参数: {}", JSONObject.toJSONString(arguments, excludefilter));
        // SerializerFeature.WriteMapNullValue 这个会打印为null的属性
        LOG.info("返回结果: {}", JSONObject.toJSONString(result, excludefilter, SerializerFeature.WriteMapNullValue));
